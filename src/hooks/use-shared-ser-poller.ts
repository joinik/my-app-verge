...
// define SharedPolerEntry type
type SharedPolerEntry = {
  subscribers: number;
  timer: number | null;
  interval: number;
  callback: (() => void) | null;
...
const sharedPollers = new Map<string, SharedPolerEntry>();
...
const ensureTimer = (key: string, entry: SharedPolerEntry) => {
