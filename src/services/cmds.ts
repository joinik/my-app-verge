...
export const patchProfilesConfig = async (profiles: IProfilesConfig) => {
  if (!isTauri()) {
    return;
  }
...
export const viewProfile = async (index: string) => {
  if (!isTauri()) {
    return;
  }
...
export const importProfile = async (url: string, option?: IProfileOption) => {
  if (!isTauri()) {
    return;
  }
...
export const reorderProfile = async (activeId: string, overId: string) => {
  return invoke<void>("reorder_profile", { activeId, overId });
};
