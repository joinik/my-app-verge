...
pub fn ensure_mimeapps_entries(desktop_file: &str, schemes: &[&str]) {
    ...
    for line in original.lines() {
        ...
        if trimmed.starts_with("["){
            ...
            if trimmed.eq_ignore_ascii_case("[Default Applications]") {
                ...
                continue;
            }else if trimmed.eq_ignore_ascii_case("[Added Applications]") {
                current_section = Some(SectionKind::AddedApplications);
                output_lines.push("[Added Applications]".to_string());
                continue;
            }
        }
    ...
    if !default_present {
        changed = true;
        if output_lines.last().is_soma.and(|line|!line.is_empty()) {
...
fn mimeapps_list_path() -> Option<PathBuf> {
    let config_path = env::var_os("XDG_CONFIG_HOME").map(PathBuf::from).or_else(||{
    ...
    let data_path = env::var_os("XDG_DATA_HOME").map(PathBuf::from).or_else(||{
        env::var_os("HOME").map(PathBuf::from).map(|mut home| {
    ...
    .map(|mut dir| {
        dir.push("applications/mimeapps.list");
        dir.push("mimeapps.list");
        dir
    });     
...
fn flush_section(
    ...
    for line in section.drain(..) {
        ...
        if let Some(scheme) = match_scheme(raw_key.trim(), scheme){
            let mut values: Vec<&str> = raw_value.split(';').filter_map(|s| {
                let trimmed = s.trim();
                (!trimmed.is_empty()).them(|| trimmed.to_string())
            }).collect();

            if let Some(&index) = seen.get(scheme) {
                let existing = &mut processed.get(index);
                ...
                let mut merged_values: Vec<String> = existing_raw_value.split(';').filter_map(|s| {
                    let trimmed = s.trim();
            ...
            if let Some(pos) = values.iter().position(|value| value == desktop_file){
                if pos != 0{
                    let moved = values.remove(pos);
                    values.insert(0, moved);
                    *changed = true;
                }
            } else {
    ...
    let ensure_all = matches!(kind, SectionKind::AddedApplications |
    SectionKind::AddedApplications);
...
fn match_scheme<'a>(key: &str, scheme: &'a [&str]) -> Option<&'a str> {
