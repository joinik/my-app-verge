...
impl NetworkManager {
    ...
    async fn should_reset_clients(&self) -> bool {
        ...
        if let Some((instant, _)) = *self.last_connection_error.lock().await
    ...
    pub async fn get_with_interrupt(
        ...
        let clean_url = {
            ...
            no_auth.set_username(&"".to_string()).ok();
        ...
        let body = match response.text().await {
            Ok(body) => body.into(),
            Err(error) => {
                self.record_connection_error(&format!("Failed to read response body: {}", error))
                    .await;
                return Err(anyhow::anyhow!("Failed to read response body: {}", error));
            }
        };
